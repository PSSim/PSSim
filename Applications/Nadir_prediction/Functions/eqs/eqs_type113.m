...
vr    = z(pointer_sv(i)+4);
xg1   = z(pointer_sv(i)+5);
xg2   = z(pointer_sv(i)+6);
xg3   = z(pointer_sv(i)+7);
...
if (genout == i)
    ...
    % Simplified Excitation System (SEXS_PTI) (Efd, Vr)
    out(pointer_sv(i)+3) = (1/TE)*(KA*(vr + (TA/TB)*(Vref-abs(V_ph(k)))) - efd);
    out(pointer_sv(i)+4) = (1/TB)*((1-TA/TB)*(Vref-abs(V_ph(k))) - vr);
    
    % GAST governor/turbine model (xg1, xg2, xg3)
    aux1  = Pc-K1*(w-1); aux2 = At + Kt*(At-xg3); 
    out(pointer_sv(i)+5) = (1/T1)*(min(aux1,aux2)-xg1);
    out(pointer_sv(i)+6) = (1/T2)*(xg1_sat-xg2);
    out(pointer_sv(i)+7) = (1/T3)*(xg2-xg3);
